...
export default function TaskEntry(props) {
  ...
  const formatDateForInput = (date) => {
    ...
    try {
      ...
      if (!day || month === 0) throw new Error("Invalid date format");
  ...
  const [descrip, setDescrip] = useState(props.updateDescrip || "");
  const [datee, setDate] = useState(formatDateForInput(props.updateDate));
  const [timee, setTime] = useState(formatTimeForInput(props.updateTime));

  useEffect(() => {
    setTitle(props.updateTitle || "");
    setDescrip(props.updateDescrip || "");
    setDate(formatDateForInput(props.updateDate));
    setTime(formatTimeForInput(props.updateTime));
  }, [
    props.updateTitle,
    props.updateDescrip,
    props.updateDate,
